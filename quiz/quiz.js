...
const resultScreen = document.querySelector('.resulte');
...
const showQuestion = (qNumber) => {
  ...
  selectedAnswered = null;
  question.textContent = data[qNumber].question;
  answerContainer.innerHTML = data[qNumber].answers.map((item, index)=>{
    return `
    <div class="answer">  
      <input type="radio" name="name1" id="${index}" value="${item.isCorrect}">
...
const selectAnswer = () => {
  answerContainer.querySelectorAll('input').forEach((el)=>{
    el.addEventListener('click', (e)=>{
      selectedAnswered = e.target.value;
    })
  })
}
...
const submitAnswer = () => {
  submit.addEventListener('click', ()=>{
    if(selectedAnswered !==null){
      selectedAnswered === 'true' ? correctCount++ : wrongCount++;
      qIndex++;
      showQuestion(qIndex);
    } else {
